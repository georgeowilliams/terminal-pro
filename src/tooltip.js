...
export function createTooltipController({ glossary, normalizeTerm }) {
  const glossaryByKey = new Map(glossary.map((g) => [g.term.toLowerCase(), g]));
  const el = document.getElementById("termTooltip");

  const close = () => el.classList.remove("open");
  const position = (anchor, centered = false) => {
    const pad = 10; const r = el.getBoundingClientRect();
    if (centered) { el.style.left = `${Math.max(pad, (window.innerWidth - r.width) / 2)}px`; el.style.top = `${Math.max(pad, (window.innerHeight - r.height) / 2)}px`; return; }
    const a = anchor.getBoundingClientRect();
    let left = a.right + 8; let top = a.top - r.height - 8;
    if (left + r.width > window.innerWidth - pad) left = a.left - r.width - 8;
    if (top < pad) top = a.bottom + 8;
    el.style.left = `${Math.max(pad, left)}px`; el.style.top = `${Math.max(pad, top)}px`;
  };

  const open = (termKey, anchor, opts = {}) => {
    const item = glossaryByKey.get(termKey); if (!item) return;
    el.innerHTML = `<div class="tooltip-head"><h3 class="tooltip-title">${escapeHtml(item.term)}</h3><button class="tooltip-close" type="button">✕</button></div>
    ...
    el.querySelectorAll(".see-chip").forEach((btn) => btn.addEventListener("click", () => {
      const k = normalizeTerm(btn.dataset.see);
  ...
  return { open, close, glossaryByKey };
}
